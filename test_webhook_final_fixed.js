// Final webhook test with correct schema
import https from 'https';

async function testWebhookFinalFixed() {
  console.log('🚀 [FINAL FIXED TEST] Testing webhook with correct schema...');
  console.log('');

  // Test data with correct schema
  const testData = {
    id: 'evt_test_final_fixed',
    object: 'event',
    type: 'checkout.session.completed',
    data: {
      object: {
        id: 'cs_test_final_fixed',
        ...
        metadata: {
          ...
          payment_id: 'test-payment-final-fixed',
...
// Test function
async function main() {
  console.log('🚀 [DEBUG] Starting final fixed webhook test...');
  console.log('');
  
  await testWebhookFinalFixed();
  
  console.log('');
  console.log('📋 [NEXT STEPS]:');
  console.log('1. Run the SQL script to test appointment creation: test_appointment_insert.sql');
  ...
  console.log('- Fixed column names: date, time, email (removed payment_id)');
  ...
  console.log('- appointments table: id, doctor_id, date, time, duration_minutes, parent_name, email, concerns');
  console.log('- Removed payment_id column (does not exist)');
  console.log('- Using correct column names for database insert');
}
