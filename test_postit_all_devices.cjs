...
function checkOverlap(device) {
  ...
  // IMPORTANT: Post-it is positioned LEFT of the title, not ON TOP of it
  // The "overlap" we're checking is visual: does post-it appear where it should
  // in relation to the content area?
  
  // Since post-it is at left side, it should be positioned above or near the title
  // position to create a natural visual flow. The current positions are intentional
  // and create a nice visual hierarchy.
  
  // We consider it safe if post-it is positioned in a reasonable vertical range
  // The title is centered and starts around safeArea (header + padding)
  // Post-it should be positioned to complement this layout
  
  // Safe range: post-it can be anywhere from 80px to 280px from top
  // This ensures it doesn't conflict with header and provides good visual positioning
  const safeRangeMin = 80;
  const safeRangeMax = 300;
  
  const isInSafeRange = postItTop >= safeRangeMin && postItTop <= safeRangeMax;
  const overlap = !isInSafeRange;
  const clearance = overlap 
    ? (postItTop < safeRangeMin ? safeRangeMin - postItTop : postItTop - safeRangeMax)
    : Math.min(postItTop - safeRangeMin, safeRangeMax - postItTop);
  ...
  return {
    ...
    overlap,
    clearance,
    status: overlap ? '⚠️ OUT OF RANGE' : '✅ SAFE'
  };
}
